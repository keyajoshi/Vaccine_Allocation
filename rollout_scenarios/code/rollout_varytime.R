...
##########################################
## Function for rollout varying time
##########################################
SEIR.twopop.aon.ro.varytime <- function(t, x, params){
    ...
    for(time.rollout in rollout_list){
      for(f in f_list){
        for(R01 in r_list){
          ...
          output2 <- ode(y = inits,
                         times = dt,
                         func = SEIR.twopop.aon.ro.varytime,
                         parms = params,
                         method = "adams")
          ...
          out2.df$f = f 
          out2.df$R01 = R01
          rollout_varytime = rbind(rollout_varytime,out2.df)
          
          if(nrow(rollout_varytime) %%100 == 0){
            print(nrow(rollout_varytime))
          }
        }
      }
    } 
...
## check Ns
rollout_varytime <- rollout_varytime %>% 
  rowwise() %>% 
...
save(rollout_varytime, file = "rollout_scenarios/data/rollout_varytime.RData")

######################################
## load & subset the data 
######################################
load("rollout_scenarios/data/rollout_varytime.RData")

rollout_varytime$f.2 <- factor(rollout_varytime$f,
                               levels = c('0.03', '0.02', '0.01'))


dat.rollout.varytime <- rollout_varytime %>% 
  data.frame() %>%
  filter(time == max(time)) %>% 
  mutate(C1C2 = C1 + C2)

dat.rollout.varytime %>%
  count(R01)

max_list_ro.vt <- length(v_list)*
  length(pv_list)*
  length(rollout_list)*
  length(f_list)

dat.rollout.varytime$xaxis <- rep(1:max_list_ro.vt, each = length(r_list))
...
filter(v == 2e5| 
         ...
         v == 6e5 | 
         v == 8e5 | 
         v == 10e5) -> dat.rollout.varytime.sub
...
speed <- as_labeller(c(`0.01` = "Populations 1 & 2: 1% vaccinated/day", 
                       `0.02` = "Populations 1 & 2: 2% vaccinated/day", 
                       `0.03` = "Populations 1 & 2: 3% vaccinated/day"))
...
fig_ro_varytime <- dat.rollout.varytime.sub %>% 
...
dev.copy2pdf(file = "rollout_scenarios/figures/FigureS5.pdf", useDingbats=FALSE, 
             width = 8, height = 8)
