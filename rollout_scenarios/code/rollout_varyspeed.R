...
## rollout where we allow the speed 
## of vaccine rollout to differ between 
## the two populations with population 2 
## having double the speed of rollout 
...
##########################################
## Function for rollout varying speed  
##########################################
SEIR.twopop.aon.ro.varyspeed <- function(t, x, params){
    ...
    for(time.rollout in rollout_list){
      for(f in f_list){
        for(R01 in r_list){
          ...
          output2 <- ode(y = inits,
                         times = dt,
                         func = SEIR.twopop.aon.ro.varyspeed,
                         parms = params,
                         method = "adams")
          ...
          out2.df$f = f 
          out2.df$R01 = R01
          rollout_varyspeed = rbind(rollout_varyspeed,out2.df)
          
          if(nrow(rollout_varyspeed) %%100 == 0){
            print(nrow(rollout_varyspeed))
          }
        }
      }
    } 
...
## check Ns
rollout_varyspeed <- rollout_varyspeed %>% 
  rowwise() %>% 
...
save(rollout_varyspeed, file = "rollout_scenarios/data/rollout_varyspeed.RData")

######################################
## load & subset the data
######################################
load("rollout_scenarios/data/rollout_varyspeed.RData")

rollout_varyspeed$f.2 <- factor(rollout_varyspeed$f, 
                                levels = c('0.03', '0.02', '0.01'))


dat.rollout.varyspeed <- rollout_varyspeed %>% 
  data.frame() %>%
  filter(time == max(time)) %>% 
  mutate(C1C2 = C1 + C2)

dat.rollout.varyspeed %>%
  count(R01)

max_list_ro_vs <- length(v_list)*
  length(pv_list)*
  length(rollout_list)*
  length(f_list)

dat.rollout.varyspeed$xaxis <- rep(1:max_list_ro_vs, each = length(r_list))
...
filter(v == 2e5| 
         ...
         v == 6e5 | 
         v == 8e5 | 
         v == 10e5) -> dat.rollout.varyspeed.sub
...
fig_ro_varyspeed <- dat.rollout.varyspeed.sub %>% 
...
dev.copy2pdf(file = "rollout_scenarios/figures/FigureS6.pdf", useDingbats=FALSE, 
             width = 8, height = 8)
