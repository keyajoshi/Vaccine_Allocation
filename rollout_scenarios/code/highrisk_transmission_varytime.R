##########################################
## This script generates figures S13
...
##############
## Function for high risk transmission rollout vary time
##############
SEIR_AN_HIGHLOW_ro_vt <- function(t, x, params){
...
highrisk_transmission_rollout_varytime <- data.frame()
      ...
      for(f in f_list){
        for(ph1 in ph_list){
          for(r.val in r_list){
            ...
            out2.df$f = f 
            out2.df$R = r.val
            highrisk_transmission_rollout_varytime = rbind(highrisk_transmission_rollout_varytime,out2.df)
            
            if(nrow(highrisk_transmission_rollout_varytime) %%100 == 0){
              print(nrow(highrisk_transmission_rollout_varytime))
...
highrisk_transmission_rollout_varytime <- highrisk_transmission_rollout_varytime %>% 
  rowwise() %>% 
...
save(highrisk_transmission_rollout_varytime,
     file = "rollout_scenarios/data/highrisk_transmission_rollout_varytime.RData")

#########################
## load & subset the data  
########################
load("rollout_scenarios/data/highrisk_transmission_rollout_varytime.RData")

highrisk_transmission_rollout_varytime$f.2 <- factor(highrisk_transmission_rollout_varytime$f, 
                                           levels = c('0.03', '0.02', '0.01'))

highrisk_transmission_rollout_varytime %>% 
  mutate(C1C2 = C1H + C2H + C1L + C2L, 
         C1C2H = C1H + C2H, 
         C1C2L = C1L + C2L) -> dat.highrisk.t.rollout.vt

dat.highrisk.t.rollout.vt %>%
  count(R)


max_list_ro_ht.vt <- length(v_list) *
  length(pv_list) * 
  length(rollout_list) *
  length(f_list) * 
  length(ph_list)

dat.highrisk.t.rollout.vt$xaxis <- rep(1:max_list_ro_ht.vt,
                     each = length(r_list))
...
rollout_hr.vt <- dat.highrisk.t.rollout.vt %>% 
  filter(ph1 == 0.5) %>% 
...
dev.copy2pdf(file = "rollout_scenarios/figures/FigureS13.pdf", useDingbats=FALSE, 
             width = 8, height = 11)
