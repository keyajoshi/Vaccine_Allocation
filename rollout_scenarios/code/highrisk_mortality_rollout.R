##########################################
## This script generates figures S10 and S12
...
N2 <- 1e6 ## population 2 size (set to be same-symmetric) 
...
##########################################
## Function for high risk mortality rollout 
##########################################
SEIR_AN_HIGHLOW_DEATHS_ro <- function(t, x, params){
...
highrisk_mortality_rollout <- data.frame()
      ...
      for(f in f_list){
        for(ph1 in ph_list){
          for(r.val in r_list){
            ...
            output2 <- ode(y = inits,
                           times = dt,
                           func = SEIR_AN_HIGHLOW_DEATHS_ro,
                           method = "adams",
                           parms = params)
            ...
            out2.df$f = f 
            out2.df$R = r.val
            highrisk_mortality_rollout = rbind(highrisk_mortality_rollout,out2.df)
            
            if(nrow(highrisk_mortality_rollout) %%100 == 0){
              print(nrow(highrisk_mortality_rollout))
            }
          }
        }
      }
...
highrisk_mortality_rollout <- highrisk_mortality_rollout %>% 
  rowwise() %>% 
...
save(highrisk_mortality_rollout , 
     file= "rollout_scenarios/data/highrisk_mortality_rollout.RData")

############################
## load & subset the data 
############################
load("rollout_scenarios/data/highrisk_mortality_rollout.RData")

highrisk_mortality_rollout$f.2 <- factor(highrisk_mortality_rollout$f, 
                                         levels = c('0.03', '0.02', '0.01'))

highrisk_mortality_rollout %>% 
  filter(time == max(time)) %>% 
  mutate(D1D2 = (D1H + D2H + D1L + D2L)) %>% 
  mutate(D1D2H = (D1H + D2H), 
          D1D2L = (D1L + D2L)) -> dat.highrisk.m.rollout

dat.highrisk.m.rollout %>%
  count(R)

max_list_ro_hd <- length(v_list) *
  length(pv_list) *
  length(rollout_list) * 
  length(f_list) *
  length(ph_list)

dat.highrisk.m.rollout$xaxis <- rep(1:max_list_ro_hd, 
                     each = length(r_list))

#############################
## Figure S10
## 25% high risk mortality 
#############################
...
speed <- as_labeller(c(`0.03` = "3% vaccinated/day", 
                       `0.02` = "2% vaccinated/day", 
                       `0.01` = "1% vaccinated/day"))

rollout <- as_labeller(c(`1` = "1 day",
                         `10` = "10 days", 
                         `30` = "30 days" , 
                         `50` = "50 days", 
                         `100` = "100 days"))

rollout_hr_m.25 <- dat.highrisk.m.rollout %>% 
  ...
  scale_y_continuous(breaks= seq(0,4000,1000), labels = seq(0,4000,1000)) 

rollout_hr_m.25

dev.copy2pdf(file = "rollout_scenarios/figures/FigureS10.pdf", useDingbats=FALSE, 
             width = 8, height = 11)

#############################
## Figure S12 
## 50% population high risk mortality 
#############################

rollout_hr_m.50 <- dat.highrisk.m.rollout %>% 
  group_by(v, pv, D1D2, ro, f.2, R, xaxis) %>% 
  filter(ph1 == 0.5) %>% 
  filter(ro != 100) %>%
  summarize() %>%
...
dev.copy2pdf(file = "rollout_scenarios/figures/FigureS12.pdf", useDingbats=FALSE, 
             width = 8, height = 11)