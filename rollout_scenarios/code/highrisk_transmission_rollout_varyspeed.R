##########################################
## This script generates figures S15
...
## We allow timing of spped to vary between the two 
## populations 
##########################################
...
dt <- seq(0,365 *years, 1) ## daily for 3 year

##############
## function
##############
SEIR_AN_HIGHLOW_ro_vs <- function(t, x, params){
  
  with(as.list(c(params,x)),{
    
    N1H = S1H + E1H + I1H + R1H + VC1H ##think about this. 
    N2H = S2H + E2H + I2H + R2H + VC2H
    
    N1L = S1L + E1L + I1L + R1L + VC1L
    N2L = S2L + E2L + I2L + R2L + VC2L
    ...
    if(t > time.rollout) {
      ...
      if(t > time.rollout) {
        ...
        if(N2*ph2*f2 <= v*(1-pv1)){
          ...
          dS2H  = -(beta2HH * S2H * I2H) -
            (beta2HL * S2H * I2L) - ((f*2) * S2H)
          
          dR2H = (nu * I2H) - ((f*2) * R2H)
          
          dVC2H = ((f*2) * S2H) + ((f*2) * R2H)
          ...
          if(VC2H > N2*ph2*f2*(1-tau)){
            ...
            dS2L = -(beta2LL * S2L * I2L) - 
              (beta2LH * S2L * I2H) - ((f*2)*S2L)
            
            dR2L =  (nu * I2L) - ((f*2)*R2L)
            
            dVC2L = ((f*2)*S2L) + ((f*2)*R2L)
    ...
    states = c(dS1H, dE1H, dI1H, dR1H, dC1H, dVC1H,
               dS1L, dE1L, dI1L, dR1L, dC1L, dVC1L,
               dS2H, dE2H, dI2H, dR2H, dC2H, dVC2H,
               dS2L, dE2L, dI2L, dR2L, dC2L, dVC2L)
...
highrisk_transmission_rollout_varyspeed <- data.frame()
      ...
      for(f in f_list){
        for(ph1 in ph_list){
          for(r.val in r_list){
            ...
            ## check again
            r0 = matrix(data = c(RHH, RLH, RHL, RLL), 
            ...
            out2.df$f = f 
            out2.df$R = r.val
            highrisk_transmission_rollout_varyspeed = rbind(highrisk_transmission_rollout_varyspeed,out2.df)
            
            if(nrow(highrisk_transmission_rollout_varyspeed) %%100 == 0){
              print(nrow(highrisk_transmission_rollout_varyspeed))
...
highrisk_transmission_rollout_varyspeed <- highrisk_transmission_rollout_varyspeed %>% 
  rowwise() %>% 
  mutate(N1 = sum(c(S1H, VC1H, E1H, I1H, R1H,
                    S1L, VC1L, E1L, I1L, R1L)),
         N1H = sum(c(S1H, VC1H, E1H, I1H, R1H)),
         N1L = sum(c(S1L, VC1L, E1L, I1L, R1L)),
         N2 = sum(c(S2H, VC2H, E2H, I2H, R2H,
                    S2L, VC2L, E2L, I2L, R2L)),
         N2H = sum(c(S2H, VC2H, E2H, I2H, R2H)),
         N2L = sum(c(S2L, VC2L, E2L, I2L, R2L))) %>%
  mutate(vtot = VC1H + VC1L + VC2H + VC2L,
         vtau = v * (1-tau),
         vtotH = VC1H + VC2H,
         vtotL = VC1L + VC2L)

save(highrisk_transmission_rollout_varyspeed,
     file = "rollout_scenarios/data/highrisk_transmission_rollout_varyspeed.RData")

#########################
## load & subset data  
########################
load("rollout_scenarios/data/highrisk_transmission_rollout_varyspeed.RData")

highrisk_transmission_rollout_varyspeed$f.2 <- factor(highrisk_transmission_rollout_varyspeed$f, 
                                           levels = c('0.03', '0.02', '0.01'))

highrisk_transmission_rollout_varyspeed %>% 
  mutate(C1C2 = C1H + C2H + C1L + C2L, 
         C1C2H = C1H + C2H, 
         C1C2L = C1L + C2L) -> dat.highrisk.t.rollout.vs

dat.highrisk.t.rollout.vs %>%
  count(R)


max_list_ro_ht.vs <- length(v_list) *
  length(pv_list) * 
  length(rollout_list) *
  length(f_list) * 
  length(ph_list)

dat.highrisk.t.rollout.vs$xaxis <- rep(1:max_list_ro_ht.vs,
                     each = length(r_list))
...
rollout <- as_labeller(c(`10` = "Populations 1 & 2: 10 days", 
                         `30` = "Populations 1 & 2: 30 days" , 
                         `50` = "Populations 1 & 2: 50 days", 
                         `100` = "Populations 1 & 2: 100 days"))

rollout_hr.vs <- dat.highrisk.t.rollout.vs %>% 
  filter(ph1 == 0.5) %>% 
  group_by(v, pv, C1C2, ro, f.2, R, xaxis) %>% 
  summarize() %>% 
  filter(ro != 1) %>%
  ggplot(aes(x = xaxis, 
             y = C1C2, 
  ...
  scale_y_continuous(breaks = seq(0,20e5, by = 4e5), labels = seq(0,20e5, by = 4e5)) + 
  guides(
...
dev.copy2pdf(file = "rollout_scenarios/figures/FigureS15.pdf", useDingbats=FALSE, 
             width = 8, height = 11)
