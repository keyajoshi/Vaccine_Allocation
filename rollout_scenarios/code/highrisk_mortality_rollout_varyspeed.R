##########################################
## This script generates figures S16
...
###########################
## Function for high risk mortality rollout vary speed
###########################

SEIR_AN_HIGHLOW_DEATHS_ro_vs <- function(t, x, params){
...
highrisk_mortality_rollout_varyspeed <- data.frame()
      ...
      for(f in f_list){
        for(ph1 in ph_list){
          for(r.val in r_list){
            ...
            output2 <- ode(y = inits,
                           times = dt,
                           func = SEIR_AN_HIGHLOW_DEATHS_ro_vs,
                           method = "adams",
                           parms = params)
            ...
            out2.df$f = f 
            out2.df$R = r.val
            highrisk_mortality_rollout_varyspeed  = rbind(highrisk_mortality_rollout_varyspeed ,out2.df)
            
            if(nrow(highrisk_mortality_rollout_varyspeed ) %%100 == 0){
              print(nrow(highrisk_mortality_rollout_varyspeed ))
            }
          }
        }
      }
...
highrisk_mortality_rollout_varyspeed  <- highrisk_mortality_rollout_varyspeed %>% 
  rowwise() %>% 
...
save(highrisk_mortality_rollout_varyspeed  , 
     file= "rollout_scenarios/data/highrisk_mortality_rollout_varyspeed.RData")

############################
## load & subset data
############################
load("rollout_scenarios/data/highrisk_mortality_rollout_varyspeed.RData")

highrisk_mortality_rollout_varyspeed$f.2 <- factor(highrisk_mortality_rollout_varyspeed$f, 
                                                     levels = c('0.03', '0.02', '0.01'))

highrisk_mortality_rollout_varyspeed %>% 
  filter(time == max(time)) %>% 
  mutate(D1D2 = (D1H + D2H + D1L + D2L)) %>% 
  mutate(D1D2H = (D1H + D2H), 
          D1D2L = (D1L + D2L))  -> dat.highrisk.m.rollout.vs

dat.highrisk.m.rollout.vs %>%
  count(R)

max_list_ro_hd.vs <- length(v_list) *
  length(pv_list) *
  length(rollout_list) * 
  length(f_list) *
  length(ph_list)

## mutate wasn't working for some reason 
dat.highrisk.m.rollout.vs$xaxis <- rep(1:max_list_ro_hd.vs, 
                     each = length(r_list))
...
rollout <- as_labeller(c(`1` = "Populations 1 & 2: 1 day", 
                         `10` = "Populations 1 & 2: 10 days", 
                         `30` = "Populations 1 & 2: 30 days", 
                         `50` = "Populations 1 & 2: 50 days", 
                         `100` = "Populations 1 & 2: 100 days"))


rollout_hr_m.vs <- dat.highrisk.m.rollout.vs %>% 
...
dev.copy2pdf(file = "rollout_scenarios/figures/FigureS16.pdf", useDingbats=FALSE, 
             width = 8, height = 11)