##########################################
## This script generates figures S9 and S11
## This scenario considers rollout of vaccines
## in a population with heterogeneous risk structure
## where we have individuals at higher risk of transmission. 
##########################################
...
#############################
## rollout function high risk transmission 
##############################
SEIR_AN_HIGHLOW_ro <- function(t, x, params){
...
highrisk_transmission_rollout <- data.frame()
      ...
      for(f in f_list){
        for(ph1 in ph_list){
          for(r.val in r_list){
            ...
            out2.df$f = f 
            out2.df$R = r.val
            highrisk_transmission_rollout = rbind(highrisk_transmission_rollout ,out2.df)
            
            if(nrow(highrisk_transmission_rollout ) %%100 == 0){
              print(nrow(highrisk_transmission_rollout ))
...
highrisk_transmission_rollout <- highrisk_transmission_rollout %>% 
  rowwise() %>% 
...
save(highrisk_transmission_rollout,
     file = "rollout_scenarios/data/highrisk_transmission_rollout.RData")

#########################
## get cumulative cases 
########################
load("rollout_scenarios/data/highrisk_transmission_rollout.RData")

highrisk_transmission_rollout$f.2 <- factor(highrisk_transmission_rollout$f, 
                                           levels = c('0.03', '0.02', '0.01'))

highrisk_transmission_rollout %>% 
  mutate(C1C2 = C1H + C2H + C1L + C2L, 
         C1C2H = C1H + C2H, 
         C1C2L = C1L + C2L) -> dat.highrisk.t.rollout

dat.highrisk.t.rollout %>%
  count(R)


max_list_ro_ht <- length(v_list) *
  length(pv_list) * 
  length(rollout_list) *
  length(f_list) * 
  length(ph_list)

dat.highrisk.t.rollout$xaxis <- rep(1:max_list_ro_ht,
                     each = length(r_list))
...
speed <- as_labeller(c(`0.03` = "3% vaccinated/day", `0.02` = "2% vaccinated/day", `0.01` = "1% vaccinated/day"))

rollout <- as_labeller(c(`10` = "10 days", `30` = "30 days" , `50` = "50 days", `100` = "100 days"))

rollout_hr_t.25 <- dat.highrisk.t.rollout %>% 
  filter(ph1 == 0.25) %>% 
...
dev.copy2pdf(file = "rollout_scenarios/figures/FigureS9.pdf", useDingbats=FALSE, 
             width = 8, height = 11)

#####################
## Figure S11
#####################
rollout_hr_t.50 <- dat.highrisk.t.rollout %>% 
  filter(ph1 == 0.5) %>% 
  group_by(v, pv, C1C2, ro, f.2, R, xaxis) %>% 
  summarize() %>% 
  filter(ro != 1) %>%
...
dev.copy2pdf(file = "rollout_scenarios/figures/FigureS11.pdf", useDingbats=FALSE, 
             width = 8, height = 11)